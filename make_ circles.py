...
if int(circle_size)%2 != 0 and int(circle_size) > 0 and int(circle_size) <= 45:
	...
	ahk = ";;; Make a Cirle with a diameter of: "+str(circle_size)+"\n"
	ahk += "^r::\n"
	...
	Send, /
	Sleep 250
	Send, fill ~"""+str(x1)+""" ~-2 ~"""+str(z1)+""" ~"""+str(x2)+""" ~-2 ~"""+str(z2)+""" minecraft:"""+block+"""
	Send, {Enter}\n
	"""
	...
	print("Invalid circle size (must be an odd number [1 - 45])...")
